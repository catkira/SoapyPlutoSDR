...
SoapyPlutoSDR::SoapyPlutoSDR(const SoapySDR::Kwargs &args)
    : dev(nullptr), rx_dev(nullptr), tx_dev(nullptr), decimation(false),
      interpolation(false), rx_stream(nullptr) {

    gainMode = false;
    frequencyOffset = 0;

    if (args.count("label") != 0)
        SoapySDR_logf(SOAPY_SDR_INFO, "Opening %s...",
                      args.at("label").c_str());

    if (ctx == nullptr) {
        if (args.count("uri") != 0) {

            ctx = iio_create_context(NULL, args.at("uri").c_str());
...
double SoapyPlutoSDR::get_sensor_value(struct iio_channel *chn) const {
    ...
    if (iio_channel_find_attr(chn, "input")) {
        if (iio_channel_attr_read_raw(chn, "input", buf, sizeof(buf)) > 0) {
            val = double_from_buf(buf);
        }
    } else {
        if (iio_channel_attr_read_raw(chn, "raw", buf, sizeof(buf)) > 0) {
            val = double_from_buf(buf);
        }

        if (iio_channel_find_attr(chn, "offset")) {
            if (iio_channel_attr_read_raw(chn, "offset", buf, sizeof(buf)) >
                0) {
                val += double_from_buf(buf);
            }
        }
        ...
        if (iio_channel_find_attr(chn, "scale")) {
            if (iio_channel_attr_read_raw(chn, "scale", buf, sizeof(buf)) > 0) {
                val *= double_from_buf(buf);
            }
        }
    }
...
SoapySDR::ArgInfo SoapyPlutoSDR::getSensorInfo(const std::string &key) const {
    ...
    if (dash < std::string::npos) {
        ...
        if (!dev)
            return info;
        iio_channel *chn =
            iio_device_find_channel(dev, channelStr.c_str(), false);
        if (!chn)
            return info;
...
void SoapyPlutoSDR::setAntenna(const int direction, const size_t channel,
    ...
    if (direction == SOAPY_SDR_RX) {
        ...
        iio_channel_attr_write_raw(
            iio_device_find_channel(dev, "voltage0", false), "rf_port_select",
        ...
        iio_channel_attr_write_raw(
            iio_device_find_channel(dev, "voltage0", true), "rf_port_select",
            name.c_str(), name.length());
    }
}
    ...
    if (direction == SOAPY_SDR_RX) {
        ...
        if (gainMode) {
            ...
            iio_channel_attr_write_raw(
                ...
                std::string("slow_attack").length());

        } else {

            iio_channel_attr_write_raw(
                iio_device_find_channel(dev, "voltage0", false),
                "gain_control_mode", "manual", std::string("manual").length());
        }
    }
}
...
void SoapyPlutoSDR::setGain(const int direction, const size_t channel,
    ...
    if (direction == SOAPY_SDR_RX) {
        std::lock_guard<pluto_spin_mutex> lock(rx_device_mutex);
        iio_channel_attr_write_longlong(
            iio_device_find_channel(dev, "voltage0", false), "hardwaregain",
            gain);
        ...
        gain = gain - 89;
        iio_channel_attr_write_longlong(
            iio_device_find_channel(dev, "voltage0", true), "hardwaregain",
            gain);
    }
}
...
void SoapyPlutoSDR::setFrequency(const int direction, const size_t channel,
    ...
    if (direction == SOAPY_SDR_RX) {
        ...
        std::lock_guard<pluto_spin_mutex> lock(tx_device_mutex);
        iio_channel_attr_write_longlong(
            iio_device_find_channel(dev, "altvoltage1", true), "frequency",
            freq);
    }
}
...
void SoapyPlutoSDR::setSampleRate(const int direction, const size_t channel,
    ...
    // note: sample rates below 25e6/12 need x8 decimation/interpolation or x4
    // FIR to 25e6/48, below 25e6/96 need x8 decimation/interpolation and x4
    // FIR, minimum is 25e6/384 if libad9361 is available it will load an
    // approporiate FIR.
    if (direction == SOAPY_SDR_RX) {
        std::lock_guard<pluto_spin_mutex> lock(rx_device_mutex);
        decimation = false;
        if (samplerate < (25e6 / (12 * fir))) {
            if (samplerate * 8 < (25e6 / 48)) {
                SoapySDR_logf(SOAPY_SDR_CRITICAL,
                              "sample rate is not supported.");
            } else if (samplerate * 8 < (25e6 / 12)) {
                SoapySDR_logf(SOAPY_SDR_NOTICE,
                              "sample rate needs a FIR setting loaded.");
            }
        ...
        iio_channel_attr_write_longlong(
            iio_device_find_channel(rx_dev, "voltage0", false),
            "sampling_frequency", decimation ? samplerate / 8 : samplerate);

        if (rx_stream)
            rx_stream->set_buffer_size_by_samplerate(decimation ? samplerate / 8
                                                                : samplerate);
    }
        ...
        iio_channel_attr_write_longlong(
            iio_device_find_channel(tx_dev, "voltage0", true),
            "sampling_frequency", interpolation ? samplerate / 8 : samplerate);
    }
...
SoapyPlutoSDR::getSampleRateRange(const int direction,
    ...
    // note that there are some gaps and rounding errors since we get truncated
    // values form IIO e.g. 25e6/12 = 2083333.333 is read as 2083333 but written
    // as 2083334
#ifdef HAS_AD9361_IIO
    // assume ad9361_set_bb_rate(), if available, will load x4 FIR as needed
    // below 25e6/96 needs x8 decimation/interpolation and x4 FIR, minimum is
    // 25e6/384
    results.push_back(SoapySDR::Range(25e6 / 384, 61440000));
#else
    // sample rates below 25e6/12 need x8 decimation/interpolation (or x4 FIR to
    // 25e6/48)
...
void SoapyPlutoSDR::setBandwidth(const int direction, const size_t channel,
    ...
    if (direction == SOAPY_SDR_RX) {
        std::lock_guard<pluto_spin_mutex> lock(rx_device_mutex);
        iio_channel_attr_write_longlong(
            iio_device_find_channel(dev, "voltage0", false), "rf_bandwidth",
            bandwidth);
    }
        ...
        std::lock_guard<pluto_spin_mutex> lock(tx_device_mutex);
        iio_channel_attr_write_longlong(
            iio_device_find_channel(dev, "voltage0", true), "rf_bandwidth",
            bandwidth);
    }
}
