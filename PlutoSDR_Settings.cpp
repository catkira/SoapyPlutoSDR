...
double SoapyPlutoSDR::get_sensor_value(struct iio_channel *chn) const
{
    ...
    if (iio_channel_find_attr(chn, "input")) {
        ...
        if (iio_channel_find_attr(chn, "offset")) {
            if (iio_channel_attr_read_raw(chn, "offset", buf, sizeof(buf)) > 0) {
                val += double_from_buf(buf);
            }
        }
...
void SoapyPlutoSDR::setAntenna(const int direction, const size_t channel, const std::string &name)
{
    if (direction == SOAPY_SDR_RX) {
        ...
        iio_channel_attr_write_raw(iio_device_find_channel(dev, "voltage0", false),
                                   "rf_port_select", name.c_str(), name.length());
    }

    else if (direction == SOAPY_SDR_TX) {
        std::lock_guard<pluto_spin_mutex> lock(tx_device_mutex);
        iio_channel_attr_write_raw(iio_device_find_channel(dev, "voltage0", true), "rf_port_select",
...
void SoapyPlutoSDR::setFrequency(const int direction, const size_t channel, const std::string &name,
